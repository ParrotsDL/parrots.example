...
parser = argparse.ArgumentParser(description='Benchmark PyTorch parallel Training')
...
# torch args
parser.add_argument('--disable_broadcast_buffers', action='store_true',
                    help='Disable DDP broadcast_buffers')
if torch_version >= 1.0:
    parser.add_argument('--bucket_size', default=25., type=float,
                        help='DDP bucket_size(MB)')
...
def main():
    ...
    elif torch_version >= 1.0:
        model = DDP(model, device_ids=[args.local_rank],
                    broadcast_buffers=(not args.disable_broadcast_buffers), bucket_cap_mb=args.bucket_size)
    else:
        raise Exception("Not support Pytorch version {}".format(torch_version))
