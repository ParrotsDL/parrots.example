...
import parrots
from parrots.ir import trace, save, wait_all
...
def main():
    ...
    for i, (input, target) in enumerate(train_loader):
        if i == 0:
            ...
            break
    f = open('log/trace/tr_data.pkl', 'wb')
    pickle.dump((tr_input, tr_target), f)
    logger.info("=> trace data saved")
    ...
    if cfgs.get('fusebnrelu', True):
        tr_func.optimize(fuseadd=False, fusebnrelu=True, fuseallreduce=False, adjustdealloc=False)
        logger.info("=> bnrelufuse finish")
    save(tr_func, 'log/trace/tr_func.json')
    logger.info("=> trace func saved")
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, monitor_writer, tr_func, tr_input, tr_target)
...
def train(train_loader, model, criterion, optimizer, epoch, args, monitor_writer, tr_func, tr_input, tr_target):
    ...
    for i in range(1201):
        ...
        loss, acc1, acc5 = tr_func(tr_input, tr_target)
