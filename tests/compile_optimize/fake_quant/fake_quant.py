...
def main():
    ...
    'elena_optimize': cfgs.elena_optimize,
    }
    ...
    for i, (input, target) in enumerate(train_loader):
        if i == 0:
            dummy_input = input.cuda()
            dummy_target = target.cuda()
        else:
            break
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, monitor_writer, dummy_input, dummy_target)
...
def train(train_loader, model, criterion, optimizer, epoch, args, monitor_writer, dummy_input, dummy_target):
    ...
    for i in range(1201):
        ...
        acc1, acc5 = accuracy(output, dummy_target, topk=(1, 5))
