...
def main():
    ...
    dist.init_process_group(backend="cncl", init_method="tcp://127.0.0.1:23456",
        rank=0, world_size=args.hvd)
    if args.device == 'mlu':
    ...
    if cfgs.get('label_smooth', None):
        if args.device == "mlu":
            criterion = LabelSmoothLoss(cfgs.trainer.label_smooth, cfgs.net.kwargs.num_classes).to(ct.mlu_device())
        else:
            criterion = LabelSmoothLoss(cfgs.trainer.label_smooth, cfgs.net.kwargs.num_classes).cuda()
    else:
        if args.device == "mlu":
            criterion = nn.CrossEntropyLoss().to(ct.mlu_device())
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args, monitor_writer, iter_time_list):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.device == "mlu":
            ...
            target = target.int().to(ct.mlu_device())
        else:
