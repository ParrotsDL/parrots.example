...
int PSROIMaskPoolForwardLaucher(
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(
        bottom_data.elemType().prim(), ([&] {
            PSROIMaskPoolingForward<scalar_t><<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
                output_size, bottom_data.ptr<scalar_t>(), spatial_scale, roi_scale, bin_scale, channels, height, width,
                pooled_height, pooled_width, bottom_rois.ptr<scalar_t>(), output_dim, pooled_height, top_data.ptr<scalar_t>(),
...
int PSROIMaskPoolBackwardLaucher(const DArrayLite top_diff, const float spatial_scale,
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(
        top_diff.elemType().prim(), ([&] {
            PSROIMaskPoolingBackward<scalar_t><<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
              output_size, top_diff.ptr<scalar_t>(), mapping_channel.ptr<int>(), num_rois, spatial_scale, roi_scale,
              bin_scale, channels, height, width, pooled_height, pooled_width, output_dim, bottom_diff.ptr<scalar_t>(),
