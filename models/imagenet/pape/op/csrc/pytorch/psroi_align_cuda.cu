    ...
    Tensor top_data, Tensor mapping_channel)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        bottom_data.type(), "psroi_align_forward_cuda", ([&] {
            PSROIAlignForward<scalar_t>
                <<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock>>>(
                    ...
                    bottom_rois.data<scalar_t>(), output_dim, pooled_height,
                    sampling_ratio, top_data.data<scalar_t>(),
                    mapping_channel.data<int>(), size_rois);
    }));
    ...
    Tensor bottom_diff, Tensor mapping_channel)
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        top_diff.type(), "psroi_align_backward_cuda", ([&] {
            PSROIAlignBackward<scalar_t>
            <<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock>>>(
                output_size, top_diff.data<scalar_t>(),
                    mapping_channel.data<int>(), spatial_scale,
                    channels, height, width, pooled_height,
                    pooled_width, output_dim, pooled_height,
                    sampling_ratio, bottom_diff.data<scalar_t>(),
                    bottom_rois.data<scalar_t>(), size_rois);
    }));
