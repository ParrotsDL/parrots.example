...
class DeformableConvFunction(Function):
    ...
    def forward(self, input, offset, weight, out_channels,
        ...
        ext_module.deform_conv_forward(
            ...
            kW=weight.size(2), kH=weight.size(3),
            dW=self.stride[0], dH=self.stride[1],
            padW=self.padding[0], padH=self.padding[1],
            dilationW=self.dilation[0], dilationH=self.dilation[1],
            groups=self.groups, deformable_group=self.deformable_groups)
        return output
        ...
        if not grad_output.is_cuda:
            ...
            if self.needs_input_grad[0] or self.needs_input_grad[1]:
                ...
                ext_module.deform_conv_backward_input(
                    input, weight, offset, grad_output, grad_input, grad_offset,
                    self.bufs_[0],
                    kW=weight.size(2), kH=weight.size(3),
                    dW=self.stride[0], dH=self.stride[1],
                    padW=self.padding[0], padH=self.padding[1],
            ...
            if self.needs_input_grad[2]:
                ...
                ext_module.deform_conv_backward_parameters(
                    input, offset, grad_output, grad_weight,
...
class DeformConv2d(Module):
    """
    标准的 Deformable Convolution V1，和 nn.Conv2d 接口相同。
...
class DeformableConvInOne(Module):
    """
    Deformable Convolution V1 的另一种实现，和 nn.Conv2d 接口相同，可以直接使用预训练
    的 nn.Conv2d 参数。
