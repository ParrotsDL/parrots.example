...
ext_module = ext_loader.load_ext('op_ext',
                                 ['syncbn_forward_step1',
                                  'syncbn_forward_step2',
                                  'syncbn_forward_step3',
                                  'syncbn_backward_step1',
                                  'syncbn_backward_step2'])
...
class SyncBNFunction(Function):
    ...
    def forward(self, input, running_mean, running_var, weight, bias, momentum,
        ...
        assert isinstance(input, (torch.cuda.HalfTensor, torch.cuda.FloatTensor)), \
            'only support Half or Float GPU Tensor, but {}'.format(input.type())
        n, c, h, w = input.size()
        ...
        ext_module.syncbn_forward_step3(
            ...
            running_mean, running_var, std, output, n=n, c=c, h=h, w=w,
            group_size=self.group_size, eps=self.eps, momentum=self.momentum)
        self.save_for_backward(input, mean, weight, std)
        return output

    @staticmethod
    def backward(self, grad_output):
        assert isinstance(grad_output, (torch.cuda.HalfTensor, torch.cuda.FloatTensor)), \
            'only support Half or Float GPU Tensor, but {}'.format(grad_output.type())
        ...
        ext_module.syncbn_backward_step1(
            input, mean, std, grad_output,
            weight_diff, bias_diff, n=n, c=c, h=h, w=w)
        # all reduce
        if self.group_size > 1:
        ...
        ext_module.syncbn_backward_step2(
            input, mean, weight, weight_diff,
            bias_diff, std, grad_output, input_diff, n=n, c=c, h=h, w=w)
        return input_diff, None, None, weight_diff, bias_diff, None, None, None, None
...
class SyncBatchNorm2d(Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            return SyncBNFunction.apply(input, self.running_mean, self.running_var,
    ...
    def extra_repr(self):
        return '{num_features}, eps={eps}, momentum={momentum}, affine={affine}, ' \
               'track_running_stats={track_running_stats}, group_size={group_size}'.format(**self.__dict__)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        super(SyncBatchNorm2d, self)._load_from_state_dict(
