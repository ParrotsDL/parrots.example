...
def asymmetric_linear_quantization_params(num_bits, saturation_min, saturation_max,
    ...
    # value to 'n', so the scale becomes 1
    # diff[diff == 0] = n
    # handle value error: saturation_min must be smaller than saturation_max
    diff[torch.abs(diff) < 1e-6] = n
...
def get_tensor_max_abs(t, per_dim=None):
    ...
    return torch.max(torch.abs_(min_val), torch.abs_(max_val))
...
def get_tensor_avg_max_abs(t, across_dim=None):
    ...
    return torch.max(torch.abs_(avg_min), torch.abs_(avg_max))
...
def get_tensor_mean_n_stds_max_abs(t, n_stds=1):
    ...
    # return torch.max(min_val.abs_(), max_val.abs_())
    return torch.max(torch.abs_(min_val), torch.abs_(max_val.abs))
