...
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, dw_kernel, dw_stride, dw_padding, bias=False):
        ...
        self.depthwise_conv2d = nn.Conv2d(
            in_channels, in_channels, dw_kernel, stride=dw_stride, padding=dw_padding, bias=bias, groups=in_channels)
...
class BranchSeparables(nn.Module):
    def __init__(self,
        ...
        self.norm1_name = 'bn_sep_1'
        norm1 = nn.BatchNorm2d(in_channels, eps=0.001, momentum=0.1, affine=True)
        self.norm2_name = 'bn_sep_2'
        norm2 = nn.BatchNorm2d(out_channels, eps=0.001, momentum=0.1, affine=True)
...
class BranchSeparablesStem(nn.Module):
    def __init__(self,
        ...
        self.norm1_name = 'bn_sep_1'
        norm1 = nn.BatchNorm2d(out_channels, eps=0.001, momentum=0.1, affine=True)
        self.norm2_name = 'bn_sep_2'
        norm2 = nn.BatchNorm2d(out_channels, eps=0.001, momentum=0.1, affine=True)
...
class BranchSeparablesReduction(BranchSeparables):
    def __init__(self,
                 ...
                 z_padding=1,
                 bias=False):
        BranchSeparables.__init__(self,
                                  in_channels,
                                  out_channels,
                                  kernel_size,
                                  stride,
...
class CellStem0(nn.Module):
    def __init__(self,
        ...
        norm_name = 'bn'
        norm = nn.BatchNorm2d(self.num_filters, eps=0.001, momentum=0.1, affine=True)
        self.conv_1x1.add_module(norm_name, norm)
...
class CellStem1(nn.Module):
    ...
    def forward(self, x_conv0, x_stem_0):
        ...
        # final path
        x_right = self.norm(torch.cat([x_path1, x_path2], 1))
...
class FirstCell(nn.Module):
    def __init__(self,
        ...
        self.norm_name = 'final_path_bn'
        norm = nn.BatchNorm2d(out_channels_left * 2, eps=0.001, momentum=0.1, affine=True)
        self.add_module(self.norm_name, norm)
    ...
    def forward(self, x, x_prev):
        ...
        x_path2 = self.path_2.conv(x_path2)
        # final path
        x_left = self.norm(torch.cat([x_path1, x_path2], 1))
...
class NASNetALarge(nn.Module):
    ...
    def __init__(self,
        ...
        midplanes = [self.stem_filters, 128, 768, 1536, 3072]
        # midplanes = [self.stem_filters, 256, 1024, 2048, 3072]
        self.out_planes = [midplanes[i] for i in self.out_layers]

        self.conv0 = nn.Sequential()
        self.conv0.add_module(
            'conv',
            nn.Conv2d(in_channels=3, out_channels=self.stem_filters, kernel_size=3, padding=0, stride=2, bias=False))
    ...
    def forward(self, input):
        ...
        x_conv0 = self.conv0(x)  # stride 2
        x_stem_0 = self.cell_stem_0(x_conv0)  # stride 4
        x_stem_1 = self.cell_stem_1(x_conv0, x_stem_0)  # stride 8
        ...
        x_reduction_cell_0 = self.reduction_cell_0(x_cell_5, x_cell_4)  # stride 16
        ...
        x_reduction_cell_1 = self.reduction_cell_1(x_cell_11, x_cell_10)  # stride 32
    ...
    def freeze_layer(self):
        layer0 = nn.Sequential(self.conv0)
        layer1 = nn.Sequential(self.cell_stem_0, self.cell_stem_1)
        layer2 = nn.Sequential(self.cell_0, self.cell_1, self.cell_2, self.cell_3, self.cell_4, self.cell_5,
                               self.reduction_cell_0)
        layer3 = nn.Sequential(self.cell_6, self.cell_7, self.cell_8, self.cell_9, self.cell_10, self.cell_11,
                               self.reduction_cell_1)
        layer4 = nn.Sequential(self.cell_12, self.cell_13, self.cell_14, self.cell_15, self.cell_16, self.cell_17)
...
def nasnetAlarge6_3072(penultimate_filters=3072, **kwargs):
    model = NASNetALarge(penultimate_filters=penultimate_filters, **kwargs)
    return model
