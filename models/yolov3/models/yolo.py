...
class Detect(nn.Module):
    ...
    def _make_grid(self, nx=20, ny=20, i=0):
        ...
        if check_version(torch.__version__, '1.10.0'):  # torch>=1.10.0 meshgrid workaround for torch>=0.7 compatibility
            yv, xv = torch.meshgrid([torch.arange(ny, dtype=int_dtype).to(d), torch.arange(nx, dtype=int_dtype).to(d)], indexing='ij')
        else:
...
class Model(nn.Module):
    def __init__(self, cfg='yolov3.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if nc and nc != self.yaml['nc']: # number of classes
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        # print(f"======= f: {f}, ch: {ch}, n: {n}, m: {m}, args: {args}")
...
if __name__ == '__main__':
    ...
    # Create model
    # model = Model(opt.cfg, ch=3)
    # input = torch.rand(1, 3, 640, 640)
    model = Model(opt.cfg, ch=1024)
    input = torch.rand(1, 1024, 3, 3)
    
    # from torchsummary import summary
    # summary(model, input_size=(1024, 3, 3))
    ...
    input, model = hc.save_and_load(input, model)
    model.train()
    input, model = hc.to_cuda(input, model)
    
    from torch.cuda import amp
    with amp.autocast():
        out = model(input)
        print("out len", out.__len__())
        for o in out:
            print(o.shape)
