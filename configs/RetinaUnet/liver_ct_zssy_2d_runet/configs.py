...
class configs(DefaultConfigs):
    ...
    def __init__(self, server_env=None):
        ...
        self.model = 'retina_unet_multichannelmask'
        ...
        self.num_epochs = 200
        self.num_train_batches = 400  # 400 if self.dim == 2 else 140
        self.batch_size = 16 if server_env else 2
        # 20 if self.dim == 2 else 8
        ...
        if self.val_mode == 'val_sampling':
            self.num_val_batches = 1000 // self.batch_size if self.server_env else 10
        ...
        self.class_dict = {1: 'body', 2: 'liver', 3: 'lesion'}  # 0 is background.
        self.patient_class_of_interest = 3  # patient metrics are only plotted for one class.
        self.ap_match_ious = [0.1, 0.5, 0.8]  # list of ious to be evaluated for ap-scoring.

        self.model_selection_criteria = ['lesion_dice']  # criteria to average over for saving epochs.
        ...
        {'mrcnn': self.add_mrcnn_configs,
         'mrcnn_multichannelmask': self.add_mrcnn_configs,
         ...
         'retina_unet_multichannelmask': self.add_mrcnn_configs,
         'seg_unet': self.add_mrcnn_configs,
         'prob_detector': self.add_mrcnn_configs,
         }[self.model]()

    def update_modicitaions(self):
        self.patch_size = self.patch_size_2D if self.dim == 2 else self.patch_size_3D
        self.pre_crop_size = self.pre_crop_size_2D if self.dim == 2 else self.pre_crop_size_3D

        if self.n_3D_context is not None and self.dim == 2:
            self.n_channels = len(self.channels) * (self.n_3D_context * 2 + 1)
    ...
    #     self.n_roi_candidates = 8 if self.dim == 2 else 8
    #
    ...
    def add_mrcnn_configs(self):
        ...
        # bl_rate = 6e-5 * self.batch_size ** (1/2)
        bl_rate = 1e-4 * self.batch_size ** (1/2)

        if self.num_epochs <= 10 or not self.server_env:
            self.learning_rate = [bl_rate] * self.num_epochs
        else:
            # decay = (1/3) ** (1/30)  # 0.964
            decay = (1/3) ** (1/20)  # 0.946
            # decay = (1/3) ** (1/10)  # 0.896
            milestone = {
                3: 1 * bl_rate,
                20: 1 * bl_rate,
                40: 1 * bl_rate,
            ...
            for i in range(self.num_epochs):
                step = list(milestone.keys())
                if i < step[0]:
                    cur_lr = (1 / step[0] ** 3) * bl_rate * (i + 1) ** 3
                elif (i + 1) in milestone:
                    cur_lr = milestone[i + 1]
                else:
                    cur_lr *= decay
                self.learning_rate.append(cur_lr)
            assert len(self.learning_rate) == self.num_epochs

        # [1e-4] * self.num_epochs
        ...
        if self.model == 'ufrcnn':
            ...
            self.class_specific_seg_flag = True
            self.num_seg_classes = 4 if self.class_specific_seg_flag else 2
            self.frcnn_mode = True

        if self.model.find('retina') > -1 or self.model.find('seg') > -1 or self.model == 'prob_detector':
            ...
            if self.model.find('retina_unet') > -1 or self.model.find('seg') > -1:
                self.operate_stride1 = True
