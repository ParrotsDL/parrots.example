...
# task_prefix = {}
task_groups = {'facebody': [i for i in range(0, 1)],
               'reid': [i for i in range(1, 2)],
               'hkjc_4': [i for i in range(1, 2)],
               }
task_prefix = {'neck': 'facebody',
               'bbox_head.cls_convs': 'facebody',
               'bbox_head.reg_convs': 'facebody',
               'bbox_head.retina_reg': 'facebody',
               'bbox_head.retina_cls': 'facebody'}
...
data = dict(
    ...
    train=dict(
        type='CustomTaskDataset',
        ann_file=[
            mot_root + 'mot/HKJCV3filter_20201208.pkl',
        ],
        img_prefix=[
            'sh1984:s3://parrots_model_data/objectflow_mot/HKJC/Image/frames/',
        ],
        with_ignore_bboxes=True,
        detect_task=detect_task,
        extra_task=extra_task,
