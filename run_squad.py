...
def train(args, train_dataset, train_dataloader, model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        if os.getenv('AVG_LOG') and (args.local_rank <= 0):
            with open(os.getenv('AVG_LOG'), 'a') as train_avg:
                train_avg.write('net:bert-case-based, iter:{}, cards:{}, avg_loss:{:.3f}, avg_time:{:.3f}, '.format( iter_num,
                                1 if args.local_rank == -1 else cards, epoch_loss/iter_num, avg_time))
        if os.getenv('BENCHMARK_LOG') and (args.local_rank <= 0):
            line = 'network:bert-base-cased, Batch Size:{}, device count:{}, Precision:{}, DPF mode:{}, \
                time_avg:{:.3f}s, time_var:{:.6f}, throughput(squences_per_sec):{:.1f}'
            line_after = re.sub(' +', ' ', line)
            with open(os.getenv('BENCHMARK_LOG'), 'a') as train_benchmark:
                train_benchmark.write(line_after.format(
                                               args.train_batch_size,
                                               cards,
                                               args.fp16_opt_level if args.cnmix else "O0",
                                               "ddp" if args.local_rank != -1 else "single",
                                               np.mean(batch_time_benchmark),
                                               np.var(batch_time_benchmark),
                                               args.train_batch_size * cards / np.mean(batch_time_benchmark),) + "\n")
        if args.max_steps > 0 and global_step > args.max_steps:
...
def main():
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="If > 0: set total number of eval steps to perform. Override num_train_epochs.",
    )
    ...
    if os.getenv('AVG_LOG') and (args.local_rank <= 0):
        with open(os.getenv('AVG_LOG'), 'a') as train_avg:
            train_avg.write('f1:{:.3f}'.format(results['f1']) + "\n")
